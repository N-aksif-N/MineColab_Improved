...
if getcwd() != drive_path:
  chdir(drive_path)
SERVERCONFIG = '/content/drive/MyDrive/minecraft/server_list.txt'
...
def starting():
  if st.session_state.Starting == True and st.session_state.start_button == '🛑 Stop':
    ...
    st.session_state.start_button = '🛑 Stop'

def ONLINE(player = False, status= False):
  if player:
    return 0
  if status:
...
def DOWNLOAD_FILE(content, url, path, file_name):
  # Check gate
...
def SERVERSJAR(server_type, version, all = False, jar = False):
  ...
  if all:
    ...
    if server_type == 'vanilla' or server_type=='snapshot':
      ...
      build = st.selectbox(' Type? ', tuple(list_build))
      choice = st.selectbox('Stable(st) or Snapshot(sn)? ', ('stable', 'snapshot'))
      if 'sn' in str(choice).lower(): choice = 'latest-snapshot';
...
def Install_server(server_name, server_type, version, tunnel_service, ngrok_authtoken = '', ngrok_region = '', zrok_authtoken = '', localtonet_authtoken = ''):
...
def Delete_server(server_name):
  msg = LOG(f'Deleting {server_name}...')
  # Auditing whether file is existed.
  ...
  if serverconfig['server_in_use'] == server_name:
    serverconfig['server_in_use']= serverconfig['server_list'][0]
...
def SOFTWARE(server_name , server_type , version, tunnel_service):
  drive_dir = listdir(f'{drive_path}/{server_name}')
  drive_dir.remove('tunnels')
